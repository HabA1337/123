...
bool command::parse(const char* string) {
    ...
    if (strncmp(s, "stop", 4) == 0) { s += 4; return parse_stop(s); }
...
bool command::parse_stop(const char* string) {
    const char* s = string;
    skip_space(&s);
    if (*s == ';') { type = command_type::stop; return true; }
    return false;
}
...
void command::print(FILE* fp) const {
    ...
    switch (type) {
        ...
        case command_type::stop: fprintf(fp, "STOP\n"); return;
